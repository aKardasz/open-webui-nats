...
- pipeline requests fall back to HTTP compatibility where designed or return a clear no-responder/timeout error
...
```bash
docker-compose -f docker-compose.yaml -f docker-compose.nats.yaml up -d terminal-service pipeline-runner
```
...
- registry evidence for `terminal-service.default` and `pipeline-runner.default`
...
- fallback behavior when `terminal-service` and `pipeline-runner` are stopped
