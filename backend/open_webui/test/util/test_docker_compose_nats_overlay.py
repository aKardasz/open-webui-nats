...
def test_nats_overlay_provisions_transitional_stage_b_services():
    ...
    assert {'nats', 'open-webui', 'retrieval-worker', 'automation-runner', 'pipeline-runner', 'terminal-service'} <= set(services)
...
def test_nats_overlay_worker_modes_are_explicit():
    ...
    automation_env = _normalized_env(compose['services']['automation-runner'])
    ...
    assert compose['services']['automation-runner']['command'] == ['python', '-m', 'open_webui', 'automation-runner']
    ...
    assert 'AUTOMATION_RUNNER_ONLY_MODE=true' in automation_env
    assert 'ENABLE_EMBEDDED_AUTOMATION_RUNNER=true' in automation_env
    ...
    assert any(item.startswith('WEBUI_SECRET_KEY=') and item != 'WEBUI_SECRET_KEY=' for item in automation_env)
...
def test_nats_overlay_disables_http_healthchecks_for_worker_only_services():
    ...
    for service_name in ['retrieval-worker', 'automation-runner', 'pipeline-runner', 'terminal-service']:
...
def test_stage_b_smoke_runbook_tracks_runtime_promotion_gate():
    ...
    assert 'automation-runner.default' in runbook
